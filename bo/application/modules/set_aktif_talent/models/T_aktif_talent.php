...
class T_aktif_talent extends CI_Model
{
	var $table = 't_aktif_talent';
	var $column_search = ['t.nama', 'at.created_at'];
	
	var $column_order = [null,'t.nama', 'at.created_at'];
	var $order = ['t.nama' => 'asc']; 
	...
	{
		$this->db->select("at.*, t.nama as nama_talent, t.foto, t.foto_thumb");
		$this->db->from($this->table.' at');
		$this->db->join('m_talent t', 'at.id_talent = t.id', 'left');
		$this->db->where('at.deleted_at is null');
