...
$(document).ready(function() {
    ...
    $("#sel_video").select2({
        ...
        ajax: {
            url: base_url+'set_video_konten/select_video_konten',
            dataType: "json",
            type: "GET",
...
function save()
{
    ...
    $.ajax({
        ...
        url: base_url + 'set_video_konten/add_data',
        data: data,
        dataType: "JSON",
        processData: false,
        contentType: false, 
        ...
        success: function (data) {
            if(data.status) {
                swal.fire("Sukses!!", "Berhasil Menambahkan Video", "success");
                location.reload();
            }else {
                for (var i = 0; i < data.inputerror.length; i++) 
                {
                    if (data.inputerror[i] != 'sel_video') {
