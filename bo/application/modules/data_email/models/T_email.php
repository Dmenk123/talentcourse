...
class T_email extends CI_Model
{
	var $table = 't_email';
	var $column_search = ['email.created_at','ckt.order_id', 'email.isi_email', 'ckt.email', 'ckt.nama', 'tipe_kirim'];
	...
	{
		$this->db->select("email.*, ckt.email, ckt.nama, CASE WHEN email.id_checkout is null THEN 'Manual' ELSE 'Via Konfirmasi' END as tipe_kirim");
		
		$this->db->from($this->table.' email');
		$this->db->join('t_checkout ckt', 'email.id_checkout  = ckt.id', 'left');		
			...
			if($_POST['search']['value']) 
			{
				...
				else
				{
					if($item == 'tipe_kirim') {
						 ...
						 */
						$this->db->or_like('(CASE WHEN email.id_checkout is null THEN \'Manual\' ELSE \'Via Konfirmasi\' END)', $_POST['search']['value'],'both',false);
					}
					else{
